...
namespace DragonLibrary.NewComponents
{
    ...
    public class ChargingThrow : AbilityCustomLogic, IAbilityTargetRestriction, IAbilityMinRangeProvider
    {
        ...
        public override IEnumerator<AbilityDeliveryTarget> Deliver(AbilityExecutionContext context,
            TargetWrapper targetWrapper)
        {
            ...
            UnitAttack unitAttack = new(unit)
                { IsCharge = true, MovementType = UnitAnimationActionLocoMotion.WalkSpeedType.Charge };
            unitAttack.Init(caster);
            var routine = (CombatController.IsInTurnBasedCombat()
                ? TurnBasedRoutine(caster, unit, unitAttack)
                : RuntimeRoutine(caster, unit, unitAttack, position2));
            return routine;
        }

        private static IEnumerator<AbilityDeliveryTarget> TurnBasedRoutine(UnitEntityData caster, UnitEntityData _,
            UnitAttack attack)
        {
            caster.View.AgentASP.MaxSpeedOverride = Math.Max(caster.View.AgentASP.MaxSpeedOverride.GetValueOrDefault(),
                caster.CombatSpeedMps * 2f);
            var currenTurn = Game.Instance.TurnBasedCombatController.CurrentTurn;
            currenTurn.CurrentMovementLimit = TurnController.MovementLimit.OneAction;
            var colldown = currenTurn.GetCooldown(caster);
            colldown.MoveAction = 0;
            colldown.StandardAction = 0;
            caster.Commands.Run(attack);
            while (attack.Result is not UnitCommand.ResultType.Success or UnitCommand.ResultType.Interrupt)
                yield return null;
            colldown.MoveAction = 6;
            caster.Descriptor.State.IsCharging = false;
        }

        private static IEnumerator<AbilityDeliveryTarget> RuntimeRoutine(UnitEntityData caster, UnitEntityData target,
            UnitAttack attack, Vector3 endPoint)
        {
            float maxDistance = GetMaxRangeMeters(caster) - 30f;
            float passedDistance = 0f;
            while (caster.View.MovementAgent.IsReallyMoving)
            {
                caster.View.MovementAgent.MaxSpeedOverride = Math.Max(
                    caster.View.MovementAgent.MaxSpeedOverride.GetValueOrDefault(), caster.CombatSpeedMps * 2f);
                ...
                {
                    endPoint = position;
                    caster.View.AgentASP.ForcePath(new ForcedPath(new List<Vector3> { caster.Position, endPoint }),
        ...
        public static float GetMinRangeMeters(UnitEntityData caster, [CanBeNull] UnitEntityData target)
        {
            ...
            {
                return TurnController.MetersOfFiveFootStep * 2f + GameConsts.MinWeaponRange.Meters +
        ...
        private bool CheckTargetRestriction(UnitEntityData caster, TargetWrapper targetWrapper,
    ...
    static class ChargingThrowPatches
    {
        ...
        [UsedImplicitly]
        [HarmonyPatch(typeof(PathVisualizer), nameof(PathVisualizer.UpdateChargePath))]
        [HarmonyPostfix]
        static void PathVisualizerPreviewPostfix(ref bool __result, PathVisualizer __instance)
        {
            if (__result)
                return;
            ...
            var targetPosition = targetUnitData.Position - (direction *
                                                            (UnitAttack.GetApproachRadius(caster.GetFirstWeapon(),
                                                                caster, targetUnitData) - 0.1f));
            ...
            {
                if (__instance.m_CurrentPath is ForcedPath { UserTag: "ChargePathTag" } &&
                    __instance.m_CurrentPath.vectorPath.Count == 2 &&
                    (__instance.m_CurrentPath.vectorPath[0] - startingPosition).sqrMagnitude < 0.1f &&
                    (__instance.m_CurrentPath.vectorPath[1] - targetPosition).sqrMagnitude < 0.1f)
                {
                    __result = true;
                    return;
                }
